...
func (t *tx) UpsertModel(m store.Model) error {
	_, err := t.tx.Exec(
		`INSERT INTO models (name, provider, model_id, base_url, api_key_cipher, notes, is_default, updated_at)
		 VALUES (?,?,?,?,?,?,?,?)
		...
		m.Name, m.Provider, m.ModelID, m.BaseURL, m.APIKeyCiphertext, m.Notes, boolInt(m.IsDefault), store.NowRFC3339())
	return err
}
...
func (t *tx) GetModel(name string) (store.Model, error) {
	return t.scanModel(t.tx.QueryRow(
		`SELECT name, provider, model_id, base_url, api_key_cipher, notes, is_default, updated_at
...
func (t *tx) scanModel(r rowScanner) (store.Model, error) {
	...
	var isDefault int
	err := r.Scan(&m.Name, &m.Provider, &m.ModelID, &baseURL, &m.APIKeyCiphertext, &notes, &isDefault, &m.UpdatedAt)
	if errors.Is(err, sql.ErrNoRows) {
